// app.js — заглушки для UI

function setStatus(message) {
  document.getElementById('status-bar').textContent = message;
}

function addHandler(id, description) {
  const el = document.getElementById(id);
  if (el) {
    el.addEventListener('click', () => setStatus(`Нажата кнопка: ${description}`));
  } else {
    console.warn(`Элемент #${id} не найден`);
  }
}

addHandler('add-button', 'Добавить предмет');
addHandler('load-heroes', 'Загрузить героев из файла');
addHandler('apply-filter', 'Применить фильтр');
addHandler('hide-button', 'Скрыть текущий предмет');
addHandler('delete-button', 'Удалить текущий предмет');
addHandler('draw-arrow-button', 'Нарисовать стрелку между предметами');
addHandler('add-hero-button', 'Добавить героя');
addHandler('delete-hero-button', 'Удалить текущего героя');
...
if (filterInput && filterMode) {
  filterInput.addEventListener('input', () => {
    setStatus(`Ввод фильтра: "${filterInput.value}" в режиме ${filterMode.value}`);
  });
  filterMode.addEventListener('change', () => {
    setStatus(`Сменён режим фильтрации на: ${filterMode.value}`);
  });
}
