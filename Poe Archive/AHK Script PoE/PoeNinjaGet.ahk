...
TestPoeNinjaGet(name){
	global ListThingCurrency
	q := ListThingCurrency[name]
	AddMessage(q.name . "[" . q.tradeID . "] ?/" . q.stackSize)
	AddMessage("Buy shift:" . q.shiftBuy . "; Sell shift:" . q.shiftSell)
	AddMessage("Count max buy:" . q.maxCountBuy . "; Count max sell:" . q.maxCountSell)
	AddMessage(q.sellTradePrice . "-" . q.centerPrice . "-" . q.buyTradePrice)
	AddMessage(q.name)
